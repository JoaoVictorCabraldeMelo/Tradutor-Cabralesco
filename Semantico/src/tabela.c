...
void mostra_tabela()
{
  if (first != NULL)
  {
    ...
    printf( MAG "%-88s\n" RESET, " ");
    Simbolo *tmp = first;
    printf(MAG "%-10s" YEL "|" RESET MAG "%-10s" YEL "|" RESET MAG " %-10s" YEL "|" RESET MAG "%-30s" YEL"|" RESET MAG "%-10s" YEL "|" RESET MAG "%-12s\n" RESET, "Linha", "Coluna",
           "Escopo", "ID", "Tipo", "Eh Funcao");
    while (tmp != NULL)
    {
      printf(MAG "%-10d" YEL "|" RESET MAG "%-10d" YEL "|" RESET MAG " %-10d" YEL "|" RESET MAG "%-30s" YEL"|" RESET MAG "%-10s" YEL "|" RESET MAG "%-12s\n" RESET, tmp->linha, tmp->coluna,
